...
- [ ] **xAI API key** cho Hermes body (`grok-4-1-fast-reasoning`, `provider: xai` native → `XAI_API_KEY`). Não code = Claude Code qua `!c` (OAuth riêng). Xem [BRIDGE_CLAUDE_CODE.md](BRIDGE_CLAUDE_CODE.md).
...
2. **Cấu hình model + gateway** trong `lucy/hermes/` (provider=xai → grok-4-1-fast, key=XAI_API_KEY; Telegram token qua env; KHÔNG ANTHROPIC_*).
